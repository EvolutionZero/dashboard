...
RUN mkdir /opt/selenium/opt/dashboard/cache
...
ENTRYPOINT ["java", "-Dlog.home=/opt/selenium/opt/dashboard/logs", "-Dspring.profiles.active=dev", "-Dlog.appender.ref=STDOUT" , "-Duser.timezone=GMT+08", "-Dfile.encoding=UTF-8","-jar","/opt/selenium/opt/dashboard/dashboard.jar"]